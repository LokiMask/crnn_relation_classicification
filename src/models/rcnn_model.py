...
def rcnn_forward(sent_pos, num_filters1, num_filters2, filter1_size, filter2_size, lexical):
...
class RCNNModel(BaseModel):
    def __init__(self, word_embed, data, word_dim,
        ...
        feature = rcnn_forward(sent_pos, num_filters1, num_filters2, filter1_size, filter2_size, lexical)
...
def build_train_valid_model(word_embed, train_data, test_data):
    with tf.name_scope("Train"):
        with tf.variable_scope('RCNNModel', reuse=None):
            m_train = RCNNModel(word_embed, train_data, FLAGS.word_dim,
    ...
    with tf.name_scope("Valid"):
        with tf.variable_scope('RCNNModel', reuse=True):
            m_valid = RCNNModel(word_embed, test_data, FLAGS.word_dim,
                    FLAGS.pos_num, FLAGS.pos_dim, FLAGS.num_relations,
