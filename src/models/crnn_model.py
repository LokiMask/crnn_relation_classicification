...
def cnn_forward(name, sent_pos, lexical, num_filters1, num_filters2):
  with tf.variable_scope(name):
    ...
    for filter_size in [3,4,5]:
      with tf.variable_scope('conv-%s' % filter_size):
        ...
        lstm_fw_cell = tf.contrib.rnn.BasicLSTMCell(num_units= num_filters2, state_is_tuple=True)
...
class CRNNModel(BaseModel):
  ...
  def __init__(self, word_embed, data, word_dim,
    ...
    feature = cnn_forward('cnn', sent_pos, lexical, num_filters1, num_filters2)
