...
def rnn_forward(name, sent_pos, lexical, num_filters, attention_size, layer_size,  keep_prob_rnn):
  with tf.variable_scope(name):
    inputs = sent_pos
    ...
    w_omega = tf.Variable(tf.random_normal([2 * num_filters, attention_size],stddev=0.1))
    ...
    l2_loss = tf.nn.l2_loss(w_omega) + tf.nn.l2_loss(b_omega) + tf.nn.l2_loss(u_omega)
  return feature, l2_loss
...
class RNNModel(BaseModel):
  ...
  def __init__(self, word_embed, data, word_dim,
              ...
              keep_prob, num_filters, attention_size,
              layer_size, keep_prob_rnn, lrn_rate, is_train):
    # input data
    ...
    feature , l2_loss_rnn = rnn_forward('rnn', sent_pos, lexical, num_filters, attention_size,  layer_size, keep_prob_rnn)
  ...
  with tf.name_scope("Train"):
    with tf.variable_scope('RNNModel', reuse=None):
      m_train = RNNModel( word_embed, train_data, FLAGS.word_dim,
                    ...
                    FLAGS.keep_prob, FLAGS.num_filters, FLAGS.attention_size,
                    FLAGS.layer_size, FLAGS.keep_prob_rnn, FLAGS.lrn_rate, is_train=True)
  with tf.name_scope('Valid'):
    with tf.variable_scope('RNNModel', reuse=True):
      m_valid = RNNModel( word_embed, test_data, FLAGS.word_dim,
                    FLAGS.pos_num, FLAGS.pos_dim, FLAGS.num_relations,
                    1.0, FLAGS.num_filters, FLAGS.attention_size,
                    FLAGS.layer_size, 1.0, FLAGS.lrn_rate, is_train=False)
